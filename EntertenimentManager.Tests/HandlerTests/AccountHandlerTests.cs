...
namespace EntertenimentManager.Tests.HandlerTests
{
    ...
    public class AccountHandlerTests
    {
        ...
        private readonly UpdateAccountCommand _validUpdateCommandSameEmails;
        private readonly UpdateAccountCommand _validUpdateCommandDifferentEmailsWithEmailExistent;
        private readonly UpdateAccountCommand _validUpdateCommandDifferentEmailsWithNewEmail;
        private readonly UpdateAccountCommand _validUpdateCommandRequestEmailNotExist;
        ...
        public AccountHandlerTests()
        {
            ...
            _validUpdateCommandSameEmails = new("Fulano", "fulano@email.com", "Pass123", _base64Image)
            {
                RequestEmail = "fulano@email.com"
            }; 
            _validUpdateCommandDifferentEmailsWithEmailExistent = new("Fulano", "fulano@email.com", "Pass123", _base64Image)
            {
                RequestEmail = "beltrano@email.com"
            };
            _validUpdateCommandDifferentEmailsWithNewEmail = new("Fulano", "new@email.com", "Pass123", _base64Image)
            {
                RequestEmail = "beltrano@email.com"
            };
            _validUpdateCommandRequestEmailNotExist = new("Fulano", "wrong@email.com", "Pass123", _base64Image)
            {
                RequestEmail = "wrong@email.com"
            }; ;
        ...
        {
            var res = await _accountHandler.Handle(_validUpdateCommandSameEmails);
            _result = (GenericCommandResult)res;
            Assert.IsTrue(_result.Success);
        }

        [TestMethod]
        public async Task ShouldReturnSuccessWhenEmailInformedFromADifferentRequestEmailNotExists()
        {
            var res = await _accountHandler.Handle(_validUpdateCommandDifferentEmailsWithNewEmail);
            _result = (GenericCommandResult)res;
            Assert.IsTrue(_result.Success);
        }
        ...
        [TestMethod]
        public async Task ShouldReturnFailWhenEmailInformedFromADifferentRequestEmailAlreadyExists()
        {
            var res = await _accountHandler.Handle(_validUpdateCommandDifferentEmailsWithEmailExistent);
            _result = (GenericCommandResult)res;
            Assert.IsFalse(_result.Success);
        }

        [TestMethod]
        public async Task ShouldReturnSuccessWhenRequestEmailFromUpdateCommandExists()
        {
            var res = await _accountHandler.Handle(_validUpdateCommandSameEmails);
            _result = (GenericCommandResult)res;
            Assert.IsTrue(_result.Success);
        ...
        [TestMethod]
        public async Task ShouldReturnFailWhenRequestEmailFromUpdateCommandNotExists()
        {
            var res = await _accountHandler.Handle(_validUpdateCommandRequestEmailNotExist);
            _result = (GenericCommandResult)res;
            Assert.IsFalse(_result.Success);
        }
        #endregion
